...
If you compile your own CSS with Tailwind, import the source theme rather than the precompiled bundle, then redeclare the tokens inside `@theme` so they land in the same layer as the shipped ones and regenerate any matching utilities. Tailwind comes in once, at the top. For the dark delta, use the registered `dark` variant instead of hand-writing the selector (see [Dark mode](https://stisla.dev/docs/theming#dark-mode)).
...
Wrapper overrideGhost retints too
...
Default primaryInline override
...
One more override path lives alongside the token surface: component-scoped variables (`--button-radius`, `--dialog-bg`, `--alert-tone`) that fall back to global tokens. Reach for those when you want one shape’s knob to differ from the global default. The [Styling](https://stisla.dev/docs/styling) page covers restyling existing components, and the [Optimization](https://stisla.dev/docs/vanilla/optimization) page has the recipes for forking a bundle or building a custom entry.
...
A card with a header, body, and footer to show every padding and surface tier respond to the change.

PrimaryCancel

Footer note

PrimaryNeutralTertiary
...
| Setting     | Value     | Use                                                   |
| ----------- | --------- | ----------------------------------------------------- |
| Compact     | `0.2rem`  | Information-dense apps, admin dashboards, tabular UIs |
| Default     | `0.25rem` | General-purpose product UI                            |
| Comfortable | `0.28rem` | Marketing surfaces, content-first reading flows       |

Card title

A card with a header, body, and footer to show every padding and surface tier respond to the base.
...
The base is a single length you can dial freely. `0.22rem` for a slight squeeze, `0.27rem` for a slight stretch. The whole system retunes at runtime. For tuning one component’s size without touching the global base, see the [sizing model](https://stisla.dev/docs/styling#sizing-model) in Styling.
...
Storage is at 89%. Archive old runs to free space.
